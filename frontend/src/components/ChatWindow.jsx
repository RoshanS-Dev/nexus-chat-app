...
const ChatWindow = ({ onToggleProfile, onBack, showBack, onGroupInfoClick }) => {
  ...
  if (!activeChat) {
    return (
      <div className="flex-1 flex items-center justify-center bg-nexus-chat">
        <div className="text-center p-8">
          <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center mx-auto mb-6 shadow-md">
            <span className="text-4xl">💬</span>
          </div>
          <h3 className="text-xl font-semibold text-gray-800 mb-2">NEXUS Messaging</h3>
  ...
  return (
    <div className="flex flex-col h-full min-h-0 bg-white overflow-hidden">
      <div className="flex items-center justify-between gap-1 px-2 sm:px-3 py-2 bg-nexus-panel border-b border-gray-200 shrink-0 min-w-0">
        ...
        <div className="px-3 py-3 bg-slate-50 border-b border-gray-200">
          <div className="flex items-center justify-between gap-2 mb-2 text-xs uppercase tracking-wide text-gray-500">
            ...
            <button
              ...
              className="text-nexus-primary hover:text-nexus-secondary"
            >
              Jump to message
            </button>
          </div>
      ...
      {replyTo && (
        <div className="px-3 py-2 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
          <div className="text-xs truncate">
            <span className="font-semibold text-nexus-secondary">Replying to </span>
      ...
      <div className="px-2 sm:px-3 py-2 bg-nexus-panel border-t border-gray-200 relative shrink-0 safe-bottom">
        {showEmojiPicker && (
        ...
        <form onSubmit={handleSendMessage} className="flex items-center gap-1 min-w-0">
          ...
          <input
            ...
            className="flex-1 min-w-0 px-3 py-2 bg-white border border-gray-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-nexus-primary/30"
            disabled={loading}
          />
          {messageText.trim() ? (
            <button type="submit" disabled={loading} className="p-2 sm:p-2.5 rounded-full bg-nexus-primary text-white hover:bg-nexus-primary-dark disabled:opacity-40 shrink-0">
              <Send size={17} />
            </button>
          ) : isRecording ? (
