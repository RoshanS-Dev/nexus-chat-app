...
const ModernSidebar = ({ activeTab, setActiveTab, onClose, badges = {}, expanded = false }) => {
  ...
  return (
    <div
      className={`h-full bg-nexus-sidebar border-r border-gray-200 flex flex-col shadow-sm w-full ${
        showLabels ? 'w-full' : 'w-20 lg:w-64'
      }`}
    >
      <div className="p-3 sm:p-4 border-b border-gray-100 shrink-0">
        {showLabels && <NexusLogo size="sm" className="mb-4" />}
        <button
          type="button"
        ...
        >
          <img
            ...
            alt={user?.fullName}
            className="w-10 h-10 rounded-full object-cover ring-2 ring-nexus-primary/30 shrink-0"
          />
          {showLabels && (
          ...
          return (
            <motion.button
              ...
              className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors min-w-0 ${
                active ? 'bg-nexus-primary/10 text-nexus-secondary' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
