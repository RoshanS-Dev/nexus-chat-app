    ...
    if (status === 'pending') {
      return (
        <button
          ...
          disabled={busy}
          className="px-3 py-1.5 rounded-lg text-xs font-medium bg-nexus-primary text-white hover:bg-nexus-primary-dark disabled:opacity-50 flex items-center gap-1"
        >
      ...
      <div className="p-3 sm:p-4 border-b border-gray-100 shrink-0">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-9 py-2 sm:py-2.5 bg-nexus-bg rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-nexus-primary/20"
            autoFocus
          />
          {loading && (
            <Loader
              className="absolute right-3 top-1/2 -translate-y-1/2 text-nexus-primary animate-spin"
              size={16}
            />
          )}
        </div>
        ...
        <AnimatePresence>
          {!searchQuery.trim() ? (
            <div className="flex flex-col items-center justify-center h-full p-8 text-center">
              ...
              <p className="text-gray-500">Search for users on Nexus</p>
