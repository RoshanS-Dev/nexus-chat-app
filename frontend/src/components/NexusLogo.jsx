const NexusLogo = ({ size = 'md', showText = true, className = '' }) => {
  const sizes = {
  ...
  return (
    <div className={`flex items-center gap-2 min-w-0 max-w-full ${className}`}>
      <div
        className={`${s.box} rounded-xl bg-nexus-primary flex items-center justify-center shadow-md shrink-0`}
      >
      ...
      {showText && (
        <span className={`${s.text} font-bold text-nexus-primary tracking-wide truncate`}>
          NEXUS
        </span>
      )}
    </div>
