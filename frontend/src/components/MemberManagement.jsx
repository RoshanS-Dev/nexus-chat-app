      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-4">
        {loading ? (
          <div className="flex justify-center py-12">
            <Loader className="w-6 h-6 text-nexus-primary animate-spin" />
          </div>
        ) : members.length === 0 ? (
              ...
              >
                ...
                {isOwner(member) && isAdmin && user._id !== member._id && (
                  <button
                    ...
                    disabled={actionLoading === member._id}
                    className="flex items-center gap-1 px-2 py-1 text-xs bg-nexus-primary text-white rounded hover:bg-nexus-secondary transition-colors disabled:opacity-60"
